...
function endStream(done) {
	...
	filelist.forEach((file, idx) => {
		...
		gulp.Gulp.prototype.task = (name, deps, task) => {
	...
	for (var name in tasks) {
		...
		if (tasks.hasOwnProperty(name)) {
			...
			task.subtasks.forEach((subtask) => gulp.add(subtask.name, subtask.deps, subtask.fn));

			// create the master task which is dependent on all of the subtasks
			gulp.add(name, task.subtasks.map((subtask) => subtask.name));
		}
	};
...
function hub(glob) {    
	...
	gulp.src(glob).pipe(through.obj(function() { console.log('hello'); }, endStream)).on('data', function() {});
}
