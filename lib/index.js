...
function hub(pattern) {
    // Assert `pattern` is a non-empty string (glob) or an array of strings (globs)
    var isString = typeof pattern === 'string';
    var isArray = Array.isArray(pattern);
    if ((!isString && !isArray) || (isString && pattern.length === 0)) {
        throw new TypeError('A non-empty glob pattern or array of glob patterns is required.');
    }
