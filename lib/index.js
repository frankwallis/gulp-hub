...
function hub(pattern) {
    // Assert `pattern` is a string (glob) or an array of strings (globs)
    if (typeof pattern !== 'string' && !Array.isArray(pattern)) {
        throw new TypeError('Glob pattern or array of glob patterns is required.');
    }
...
function assignUniqueNames(subfiles) {
    ...
    subfiles.forEach(function (subfile, idx) {
        var parts = path.dirname(subfile.relativePath).split(path.sep);

        parts.forEach(function (part) {
            if (names.indexOf(path) < 0)
                subfile.uniqueName = part;
        });

        if(subfile.uniqueName === '')
            subfile.uniqueName = parts.join('-');
    });
}
