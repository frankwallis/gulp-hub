...
function hub(pattern, options) {
    options = options || {};
    if(options.excludedTasks){
        var runningTask = process.argv[process.argv.length - 1];
        var isArgExcluded = _.contains(options.excludedTasks, runningTask);
        if(isArgExcluded) {
           return;
        }
    }
